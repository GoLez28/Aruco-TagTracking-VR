...
public class Parser
{
    ...
    public void Parse(byte[] buf, ref int pos, int endPos, ulong timestamp = 0x1u)
    {
        ...
        if (pos != endPos)
        {
           Console.WriteLine($"The parsed data size is inconsitent with the given size: {pos} / {endPos}");
        }
    }
    ...
    void ParseBundle(byte[] buf, ref int pos, int endPos)
    {
        ...
        while (pos < endPos)
        {
            ...
            {
                Console.WriteLine($"Given data is invalid (bundle size ({contentSize}) is not a multiple of 4).");
                pos += contentSize;
            }
        }
    }
