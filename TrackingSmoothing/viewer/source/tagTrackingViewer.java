/* autogenerated by Processing revision 1283 on 2023-11-22 */
...
public final double[] quatToMatrix(double[] q) {
  ...
  double tmp1 = q[0]*q[1];
  double tmp2 = q[2]*q[3];
  double m10 = 2.0f * (tmp1 + tmp2)*invs ;
  double m01 = 2.0f * (tmp1 - tmp2)*invs ;

  tmp1 = q[0]*q[2];
  tmp2 = q[1]*q[3];
  double  m20 = 2.0f * (tmp1 - tmp2)*invs ;
  double  m02 = 2.0f * (tmp1 + tmp2)*invs ;
  tmp1 = q[1]*q[2];
  tmp2 = q[0]*q[3];
  double  m21 = 2.0f * (tmp1 + tmp2)*invs ;
  double  m12 = 2.0f * (tmp1 - tmp2)*invs ;
