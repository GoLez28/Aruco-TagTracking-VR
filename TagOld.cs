    ...
    static class Tag {
        ...
        public class CombinedTracker {
            ...
            public Matrix4x4 Obtain() {
                ...
                int closestCam = 0;
                if (depth2 < depth1 || (updateCount[0] > 2 && updateCount[1] <= 2))
                    closestCam = 1;
                Matrix4x4 rot1 = singles[0].rot;
                Matrix4x4 rot2 = singles[1].rot;
                //Quaternion q1 = Quaternion.CreateFromRotationMatrix(singles[closestCam].rot);
                //Quaternion q2 = Quaternion.CreateFromRotationMatrix(singles[closestCam].p1_rot);
                //Quaternion q3 = Quaternion.CreateFromRotationMatrix(singles[closestCam].p2_rot);
                //float d1 = Math.Abs(q1.X - q2.X) + Math.Abs(q1.Y - q2.Y) + Math.Abs(q1.Z - q2.Z) + Math.Abs(q1.W - q2.W);
                //float d2 = Math.Abs(q2.X - q3.X) + Math.Abs(q2.Y - q3.Y) + Math.Abs(q2.Z - q3.Z) + Math.Abs(q2.W - q3.W);
                //float d3 = Math.Abs(q1.X - q3.X) + Math.Abs(q1.Y - q3.Y) + Math.Abs(q1.Z - q3.Z) + Math.Abs(q1.W - q3.W);
                //if (d2 < d1 && d2 < d3) {
                //    if (closestCam == 0) {
                //        rot1 = singles[0].p1_rot;
                //    } else if (closestCam == 1) {
                //        rot2 = singles[1].p1_rot;
                //    }
                //}
                Matrix4x4[] pos = new Matrix4x4[] {
                    ...
                    Matrix4x4.Multiply(rot2, Matrix4x4.CreateTranslation(singles[1].pos.X * cameras[0].depthMult, singles[1].pos.Y * cameras[0].depthMult, depth2 * cameras[0].depthMult))
                };
        ...
        public class ClusterTracker {
            ...
            public OneEuroFilter<Quaternion> smoothedRot = new OneEuroFilter<Quaternion>(5);
            ...
            public ClusterTracker(string name, int[] ids, Vector3[] ofss, float[] rots) {
                ...
                for (int i = 0; i < ids.Length; i++) {
                    ...
                    if (name.Equals("waist")) {
                        trackersRotationsFilter[i] = new OneEuroFilter<Quaternion>(5);
                    } else {
            ...
            public Matrix4x4 Obtain() {
                //if (!trackerName.Equals("leftfoot")) return new();
                int minId = 0;
                int minScore = 999999;
                for (int i = 0; i < updateCount.Length; i++) {
                    if (updateCount[i] <= minScore) {
                        minScore = updateCount[i];
                        minId = i;
                    }
                }
                List<int> sameCount = new List<int>();
                for (int i = 0; i < updateCount.Length; i++) {
                    if (updateCount[i] <= minScore + 1) {
                        sameCount.Add(i);
                    }
                }
                //WAIST RECENTERING
                if (trackerName.Equals("waist") && Program.allowWaistRecentering) {
                    Random rnd = new Random();
                    if (sameCount.Count == 4) {
                        Quaternion[] allRots2 = new Quaternion[sameCount.Count];
                        Vector3[] allPos2 = new Vector3[sameCount.Count];
                        Vector3[] trackerOffsets2 = new Vector3[] {
                            new Vector3(0, 0, 0),new Vector3(0, 0, 0),new Vector3(0, 0, 0),new Vector3(0, 0, 0)
                        };
                        for (int i = 0; i < sameCount.Count; i++) {
                            Vector3 pointMat = trackerOffsets2[sameCount[i]];
                            Matrix4x4 newMat = trackers[sameCount[i]].Obtain();
                            Matrix4x4 result = Matrix4x4.Multiply(Matrix4x4.CreateTranslation(pointMat), newMat);
                            Matrix4x4 rotMat = Matrix4x4.CreateFromAxisAngle(new Vector3(0, 1, 0), trackerRotations[sameCount[i]]);
                            result = Matrix4x4.Multiply(rotMat, result);
                            allRots2[i] = Quaternion.CreateFromRotationMatrix(result);
                            allPos2[i] = result.Translation;
                        }
                        Vector3 avgPos2 = (allPos2[0] + allPos2[1] + allPos2[2] + allPos2[3]) / 4;
                        for (int i = 0; i < trackerOffsets.Length; i++) {
                            trackerOffsets[i] = allPos2[i] - avgPos2;
                            Matrix4x4 inasdasd = trackers[sameCount[i]].Obtain();
                            Matrix4x4 asdasd;
                            Matrix4x4.Invert(inasdasd, out asdasd);
                            trackerOffsets[i] = Matrix4x4.Multiply(Matrix4x4.CreateTranslation(avgPos2), asdasd).Translation;
                        }
                    }
                }
                sameCount.Clear();
                ...
                int agreedCount = 0;
                List<Quaternion> bestRots = new List<Quaternion>();
                ...
                for (int i = 0; i < trackers.Length; i++) {
                    if (updateCount[i] == 1) {
                        Matrix4x4 newMat = trackers[i].Obtain();
                        Vector3 pointMat = trackerOffsets[i];
                        Matrix4x4 result = Matrix4x4.Multiply(Matrix4x4.CreateTranslation(pointMat), newMat);
                        Matrix4x4 rotMat = Matrix4x4.CreateFromAxisAngle(new Vector3(0, -1, 0), trackerRotations[i]);
                        result = Matrix4x4.Multiply(rotMat, result);
                        bool added = false;
                        for (int j = 0; j < posibleRots.Count; j++) {
                            Quaternion quaternion = result.Rotation();
                            Quaternion quaternion1 = Quaternion.Inverse(posibleRots[j]);
                            Quaternion quaternion11 = quaternion1 * quaternion;
                            float trtrtr = Quaternion.Dot(quaternion11, Quaternion.Identity);
                            if (trtrtr > 0.95f) {
                ...
                for (int i = 0; i < trackers.Length; i++) {
                    if (updateCount[i] == 1) {
                        ...
                        for (int j = 0; j < posibleRotsCountId.Count; j++) {
                        ...
                        if (updateCountp[i] == 1) {
                            Vector3 ddddds = newMat.Translation - prevMat[i].Translation;
                            availableAvgPosDiff += ddddds;
                            availableAvgPos += newMat.Translation;
                            Quaternion ddfff1 = prevMat[i].Rotation();
                            Quaternion ddfff2 = newMat.Rotation();
                            Quaternion rtrt = ddfff2 * Quaternion.Inverse(ddfff1);
                            availableAvgRot = Utils.QLerp(availableAvgRot, rtrt, availableCount == 0 ? 1f : 0.5f);
                            availableCount++;
                        }
                        if (trackers[i].allAgreed) {
                            bestRots.Add(Quaternion.CreateFromRotationMatrix(newMat));
                            agreedCount++;
                        }
                        estimatedPos[i] = newMat.Translation;
                        estimatedRot[i] = Quaternion.CreateFromRotationMatrix(newMat);
                        if (float.IsNaN(estimatedRot[i].X) || float.IsNaN(estimatedPos[i].X))
                            Console.WriteLine();
                        trackerPresence[i]--;
                        if (trackerPresence[i] < 0)
                            trackerPresence[i] = 0;
                        if (lastRotCount == 10) trackerPresence[i] = 0;
                ...
                if (availableCount == 0) {
                    for (int i = 0; i < trackers.Length; i++) {
                        if (updateCount[i] == 1) {
                            Matrix4x4 newMat = trackers[i].Obtain();
                            //Matrix4x4 rotMat = Matrix4x4.CreateFromAxisAngle(new Vector3(0, -1, 0), trackerRotations[sameCount[i]]);
                            //newMat = Matrix4x4.Multiply(rotMat, newMat);
                            Vector3 ddddds = newMat.Translation - prevPos;
                            availableAvgPosDiff += ddddds;
                            availableAvgPos += newMat.Translation;
                            //Matrix4x4 rotMat = Matrix4x4.CreateFromAxisAngle(new Vector3(0, -1, 0), trackerRotations[i]);
                            //Matrix4x4 result = Matrix4x4.Multiply(rotMat, newMat);
                            Quaternion ddfff1 = prevRot;
                            Quaternion ddfff2 = newMat.Rotation();
                            Quaternion rtrt = ddfff2 * Quaternion.Inverse(ddfff1);
                            availableAvgRot = Utils.QLerp(availableAvgRot, rtrt, availableCount == 0 ? 1f : 0.5f);
                            //Aruco.DrawAxis(Matrix4x4.Multiply(Matrix4x4.CreateFromQuaternion(rtrt * ddfff1), Matrix4x4.CreateTranslation(prevMat[i].Translation + ddddds)));
                            availableCount++;
                            //availableAvgRot = rtrt;
                            estimatedPos[i] = newMat.Translation;
                            estimatedRot[i] = Quaternion.CreateFromRotationMatrix(newMat);
                            //Aruco.DrawAxis(newMat);
                ...
                for (int i = 0; i < trackers.Length; i++) {
                    if (updateCount[i] > 1) {
                        //estimatedPos[i] = prevMat[i].Translation + availableAvgPos;
                        ...
                        estimatedRot[i] = availableAvgRot * Quaternion.CreateFromRotationMatrix(prevMat[i]);
                        trackerPresence[i]++;
                        if (trackerPresence[i] > 50)
                            trackerPresence[i] = 50;
                    }
                ...
                for (int i = 0; i < estimatedRot.Length; i++) {
                    ...
                    if (float.IsNaN(filteredRot.X)) {
                        if (trackerName.Equals("waist")) {
                            trackersRotationsFilter[i] = new OneEuroFilter<Quaternion>(20);
                        } else {
                            trackersRotationsFilter[i] = new OneEuroFilter<Quaternion>(2);
                        }
                        continue;
                ...
                //Matrix4x4 adwdw = Matrix4x4.Multiply(Matrix4x4.CreateFromQuaternion(availableAvgRot), Matrix4x4.CreateTranslation(availableAvgPos + new Vector3(0, 0, 1f)));
                //if ((Program.frameCount / 4 % 1) == 0 && trackerIndex[0] == 4 && !float.IsNaN(availableAvgRot.X) && !float.IsNaN(availableAvgPos.X))
                //    Aruco.DrawAxis(adwdw, 1);
                ...
                for (int i = 0; i < estimatedPos.Length; i++) {
                    Vector3 pointMat = trackerOffsets[i];
                    //Aruco.DrawAxis(Matrix4x4.Multiply(Matrix4x4.CreateFromQuaternion(estimatedRot[i]), Matrix4x4.CreateTranslation(estimatedPos[i])));
                    Matrix4x4 newMat = Matrix4x4.Multiply(Matrix4x4.CreateFromQuaternion(estimatedRot[i]), Matrix4x4.CreateTranslation(estimatedPos[i]));
                    Matrix4x4 result = Matrix4x4.Multiply(Matrix4x4.CreateTranslation(pointMat), newMat);
                    Matrix4x4 rotMat = Matrix4x4.CreateFromAxisAngle(new Vector3(0, -1, 0), trackerRotations[i]);
                    result = Matrix4x4.Multiply(rotMat, result);
                    poss[i] = result.Translation;
                    if (float.IsNaN(poss[i].X))
                        poss[i] = new();
                    rots[i] = result.Rotation();
                    ...
                    //if (trackerPresence[i] < minPresence) minPresence = trackerPresence[i];
                }

                Quaternion prevRotPresence = Quaternion.Identity;
                if (maxId == -1) {
                    prevRotPresence = prevRot;
                    lastRotCount++;
                } else {
                    if (posibleRotsCountId[maxId].Count == 1 && posibleRotsCountId[maxId][0] == -1) {
                        lastRotCount++;
                    } else {
                        lastRotCount = 0;
                    }
                    for (int i = 0; i < posibleRotsCount[maxId].Count; i++) {
                        Quaternion quaternion = Utils.QLerp(Quaternion.Identity, posibleRotsCount[maxId][i], 1f / posibleRotsCount[maxId].Count);
                        prevRotPresence = quaternion * prevRotPresence;
                    }
                }
                ...
                //    for (int j = i; j < rots.Length; j++) {
                //        if (i == j) continue;
                //        float rotDiff = Quaternion.Dot(Quaternion.Inverse(rots[i]) * rots[j], Quaternion.Identity);
                //        if (rotDiff > 0.99f) {
                //            Quaternion avg = Quaternion.Lerp(rots[i], rots[j], 0.5f);
                //            bestRots.Add(avg);
                //        }
                //    }
                //}
                //if (bestRots.Count + agreedCount > 0) {
                //    for (int i = 1; i < bestRots.Count; i++) {
                //        //avgBest = Quaternion.Lerp(avgBest, bestRots[i], 1f / bestRots.Count);
                //        prevRotPresence = Quaternion.Lerp(Quaternion.Identity, bestRots[i], 1f / bestRots.Count) * prevRotPresence;
                //    }
                //    lastRotCount = 0;
                //} else if (bestRots.Count == 1) {
                //    float rotDiff = Quaternion.Dot(Quaternion.Inverse(bestRots[0]) * prevRot, Quaternion.Identity);
                //    if (rotDiff > 0.85f) {
                //        prevRotPresence = bestRots[0];
                //        lastRotCount = 0;
                //    } else {
                //        if (lastRotCount > 10) {
                //            for (int i = 0; i < rots.Length; i++) {
                //                float add = Utils.GetMap(trackerPresence[i], minPosPresence, maxPosPresence, 1f / sumPosPresence, 0f);
                //                prevRotPresence = Quaternion.Lerp(Quaternion.Identity, rots[i], add) * prevRotPresence;
                //            }
                //        } else {
                //            lastRotCount++;
                //            prevRotPresence = prevRot;
                ...
                if (trackerName.Equals("waist")) {
                    smoothRot = 0.2f;
                    smoothedPos.UpdateParams(10f);
                    //trackerRotations = new float[] { 0f, (float)Math.PI * 0.3f, (float)Math.PI * 0.7f, (float)Math.PI * 1f };
                } else {

                }
                //avgPos = prevPos * (1 - smoothPos) + avgPos * smoothPos;
                ...
                Vector3 avgPos = new();
                trackerRotations = new float[] { (float)Math.PI * 1.5f, (float)Math.PI, (float)Math.PI * 0.5f, 0f };
                agreedCount = 0;
                if (sameCount.Count > 0) {
                    Quaternion[] allRots = new Quaternion[sameCount.Count];
                    bestRots = new List<Quaternion>();
                    for (int i = 0; i < estimatedPos.Length; i++) {
                        Matrix4x4 newMat = trackers[sameCount[i]].Obtain();

                        Vector3 pointMat = trackerOffsets[sameCount[i]];
                        Matrix4x4 result = Matrix4x4.Multiply(Matrix4x4.CreateTranslation(pointMat), newMat);
                        Matrix4x4 rotMat = Matrix4x4.CreateFromAxisAngle(new Vector3(0, 1, 0), trackerRotations[sameCount[i]]);
                        result = Matrix4x4.Multiply(rotMat, result);
                        allRots[i] = Quaternion.CreateFromRotationMatrix(result);
                        if (trackers[sameCount[i]].allAgreed) {
                            bestRots.Add(allRots[i]);
                            agreedCount++;
                        }
                        if (i == 0) {
                            avgPos = result.Translation;
                            avgRot = allRots[i];
                        } else {
                            avgPos = (avgPos + result.Translation) * 0.5f;
                        }
                    }
                    for (int i = 0; i < allRots.Length; i++) {
                        for (int j = i; j < allRots.Length; j++) {
                            if (i == j) continue;
                            float rotDiff = Quaternion.Dot(Quaternion.Inverse(allRots[i]) * allRots[j], Quaternion.Identity);
                            if (rotDiff > 0.99f) {
                                Quaternion avg = Quaternion.Lerp(allRots[i], allRots[j], 0.5f);
                                bestRots.Add(avg);
                            }
                        }
                    }
                    if (bestRots.Count + agreedCount > 0) {
                        Quaternion avgBest = bestRots[0];
                        for (int i = 1; i < bestRots.Count; i++) {
                            avgBest = Quaternion.Lerp(avgBest, bestRots[i], 1f / bestRots.Count);
                        }
                        avgRot = avgBest;
                            ...
                            if (lastRotCount > 10) {
                                for (int i = 0; i < allRots.Length; i++) {
                                    if (i == 0) {
                                        ...
                                        avgRot = Quaternion.Lerp(avgRot, allRots[i], 1f / sameCount.Count);
                                    }
                                }
                            } else {
                                lastRotCount++;
                ...
                //float smoothPos = 0.5f;
                smoothRot = 0.1f;
                if (trackerName.Equals("waist"))
                    smoothRot = 0.05f;
                //avgPos = prevPos * (1 - smoothPos) + avgPos * smoothPos;
                if (!float.IsNaN(avgRot.X)) {
                    //avgRot = smoothedRot.Filter(avgRot);
                    if (float.IsNaN(prevRot.X))
                        prevRot = avgRot;
                    avgRot = Quaternion.Lerp(prevRot, avgRot, smoothRot);
                    avgRot = Quaternion.Normalize(avgRot);
                }
                if (!float.IsNaN(avgPos.X))
                    avgPos = smoothedPos.Filter(avgPos);
                else
                    avgPos = new Vector3();
                prevPos = avgPos;
                prevRot = avgRot;
                Matrix4x4 finalMat = Matrix4x4.Multiply(Matrix4x4.CreateFromQuaternion(avgRot), Matrix4x4.CreateTranslation(avgPos));
                ...
                //Matrix4x4 rotMat = Matrix4x4.CreateFromAxisAngle(new Vector3(0, 1, 0), trackerRotations[sameCount[i]]);
                //result = Matrix4x4.Multiply(rotMat, result);
                //avgPos = avgPos + (result * (1f / sameCount.Count));

                //TESTING
                //if (trackerName.Equals("waist")) {
                //    int index = (Program.frameCount / 16) % trackerIndex.Length;
                ...
                //    };
                //}
                //    Vector3 pointMat = trackerOffsets[index];
                //    Matrix4x4 newMat = trackers[index].Obtain();
                //    Matrix4x4 result = Matrix4x4.Multiply(Matrix4x4.CreateTranslation(pointMat), newMat);
                //    Matrix4x4 rotMat = Matrix4x4.CreateFromAxisAngle(new Vector3(0, 1, 0), trackerRotations[index]);
                //    finalMat = Matrix4x4.Multiply(rotMat, result);
                //    if (updateCount[index] > 2)
                //        finalMat = new Matrix4x4();
                //}
                //------------------
                return finalMat;
            }
        ...
        public static void RecieveTracker(int index, int camera, Matrix4x4 rot, Vector3 pos) {
            ...
            if (Program.timer.ElapsedMilliseconds - saveMatTime < 20000) {
                ...
                if (index == 0 || index == 2 || index == 4 || index == 6) {
                    ...
                    string message = $"by exsisting rnd                              ";
